...
class _ChatRoomPageState extends State<ChatRoomPage> {
  ...
  void _startTeacherRefresh() {
    _teacherRefreshTimer = Timer.periodic(const Duration(seconds: 3), (_) async {
      ...
      // 只更新成員列表，不重新載入整個房間
      await _updateParticipantsListOnly();

      // 檢查房間狀態變化（輕量檢查）
      await _checkRoomStatusOnly();
    });
  }
  ...
  /// 只檢查房間狀態（老師端用）
  Future<void> _checkRoomStatusOnly() async {
    try {
      final token = await getToken();
      final res = await http.get(
        Uri.parse('$baseUrl/api/chat/rooms/${widget.roomId}'),
      ...
      if (res.statusCode == 200) {
        ...
        if (statusStr == 'matching') {
          newStatus = RoomStatus.matching;
        } else if (statusStr == 'active' || isStarted) {
        ...
        // 如果房間狀態變為 matching，老師也跳轉到配對結果頁
        if (newStatus == RoomStatus.matching) {
          _teacherRefreshTimer?.cancel();
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
        ...
        // 如果房間狀態變為 started，跳轉到遊戲頁面
        else if (newStatus == RoomStatus.started) {
          _teacherRefreshTimer?.cancel();
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
      ...
      debugPrint('Check room status error: $e');
    }
  }

  /// 只更新成員列表（老師端用）
  Future<void> _updateParticipantsListOnly() async {
    try {
      final token = await getToken();
      final res = await http.get(
        Uri.parse('$baseUrl/api/chat/rooms/${widget.roomId}'),
      ...
      debugPrint('Update participants only error: $e');
    }
  }
